...
final case class ItemService(itemRepo: ItemRepository):
  ...
  def addItem(input: CreateItemInput): IO[RepositoryError.DbEx | RepositoryError.Conflict, Item] =
  ...
  def updateItem(id: ItemId, input: UpdateItemInput): IO[RepositoryError.DbEx | RepositoryError.MissingEntity, Item] =
    itemRepo.update(id, input)

object ItemService:
  def addItem(input: CreateItemInput): ZIO[ItemService, RepositoryError.DbEx | RepositoryError.Conflict, Item] =
  ...
  def updateItem(id: ItemId, input: UpdateItemInput)
